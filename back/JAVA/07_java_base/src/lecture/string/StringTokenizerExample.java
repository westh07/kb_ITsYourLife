...
public class StringTokenizerExample {
    public static void main(String[] args) {
        ...
        * 1. split() : 정규표현식을 이용해 문자열을 분리, 비정형화된 문자열을 분리 할때 좋음 (공백 문자열 값 포함)
        *
        * 2. StringTokenizer : 문자열의 모든 문자들을 구분자로 하여 문자열을 분리, 정형화된 문자열 패턴을 분리할때 좋음 (공백 문자열 무시)
        * - split()보다 속도면에서 빠름!
        *
        * - ex) csv 파일 : ,로 데이터를 나눔
        * */
...
//        compareSplitAndTokenizer("emp3",emp3);
    ...
    public static void compareSplitAndTokenizer(String label, String data){
        ...
        while(tokenizer.hasMoreTokens()){
            ...
            System.out.println("StringTokenizer token : " + tokenizer.nextToken());
        }
