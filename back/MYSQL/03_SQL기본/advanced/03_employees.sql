...
where to_date = '9999-01-01' -- 재직자
and dept_no = 'd005'; -- 005부서

/*
SUBQUERIES
- 서브 쿼리는 다른 쿼리에서 실행되는 쿼리
- 테이블 내에서 존재하지 않는 조건에 근거한 값들ㅇ르 검색할 때 사용
*/
...
- 서브쿼리 결과 모든 값과 비교
ANY, SOME
- 서브쿼리 결과가 하나로도 만족하면 참
*/
-- 급여가 재직자 모두의 평균보다 크면 출력
select *
from salaries
where to_date = '9999-01-01' and salary > all ( select avg(salary)
                                                from salaries
                                                where to_date = '9999-01-01' );
select *
from salaries
where to_date = '9999-01-01' and salary > any ( select avg(salary)
                                                from salaries
                                                where to_date = '9999-01-01' );